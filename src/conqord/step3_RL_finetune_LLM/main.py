...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=[
            "/mnt/data/taoshuchang.tsc/datasets/Anthropic/hh-rlhf/helpful-base_conf_half_sharp"
        ],
        ...
        for step, batch_prompt in enumerate(eval_dataloader):
            if torch.distributed.get_rank() == 0:
                print(
                    ...
                    step,
                    "len(batch_prompt)",
                    len(batch_prompt["prompt"]),
                    "^^^^^^^^^^^^^",
                )
            batch_prompt = to_device(batch_prompt, device)

            out = trainer.generate_experience(
                batch_prompt["prompt"], batch_prompt["prompt_att_mask"], step, eval=True
            )
            if torch.distributed.get_rank() == 0:
            ...
            if torch.distributed.get_rank() == 0:
                print(
                    "^^^^^^^^^^^^^ Evaluation_actor_loss", actor_loss, "^^^^^^^^^^^^^"
                )
...
def main():
    ...
    if args.local_rank == -1:
        ...
        print(
            f"Global rank: {rank}, Local rank: {local_rank}, Current device: {torch.cuda.current_device()}"
        )
    ...
    tokenizer = load_hf_tokenizer(
        args.tokenizer_model_name_or_path, fast_tokenizer=True
    )
    (
        prompt_train_dataloader,
        prompt_eval_dataloader,
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            if exp_dataset is not None:
                ...
                if args.enable_tensorboard and torch.distributed.get_rank() == 0:
                    writer.add_scalar(
                        "train/reward", average_reward / inner_iter, global_step=step
                    )
                    writer.add_scalar(
                        "train/reward_ori",
                        average_reward_ori / inner_iter,
                        global_step=step,
                    )
                    writer.add_scalar(
                        "train/reward_align_rewards",
                        average_reward_ori / inner_iter,
                        global_step=step,
                    )
                    writer.add_scalar(
                        "train/confidence", confidence / inner_iter, global_step=step
                    )
                    writer.add_scalar("train/actor_loss", actor_loss, global_step=step)
                    # writer.add_scalar('train/actor_loss_sum',
                    #                   actor_loss_sum,
                    #                   global_step=step)
                    writer.add_scalar(
                        "train/critic_loss", critic_loss, global_step=step
                    )
            ...
            if step % 5 == 0 or save_max or save_max_val:
                ...
                if torch.distributed.get_rank() == 0:
                    save_hf_format(
                        rlhf_engine.actor, tokenizer, args, sub_folder="actor"
                    )
                    save_hf_format(
                        rlhf_engine.critic, tokenizer, args, sub_folder="critic"
                    )
                    if args.enable_ema:
                        save_hf_format(
                            rlhf_engine.actor_ema,
