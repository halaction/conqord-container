...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    if "Dahoas/rm-static" in dataset_name:
        ...
        return raw_datasets.AnthropicFullhhrlhf_confDataset(
            output_path, seed, local_rank, dataset_name
        )
    elif "Anthropic/hh-rlhf/helpful-base" in dataset_name:
        return raw_datasets.AnthropicFullhhrlhfDataset(
            output_path, seed, local_rank, dataset_name
        )
...
def create_prompt_dataset(
    ...
    print_rank_0(
        f"aaaa buf_create_cache.item() != 0: {buf_create_cache.item() != 0}",
    ...
    return torch.load(train_fname, weights_only=False), torch.load(
        eval_fname, weights_only=False
    )
...
def find_indices_of_confidence_in_seq(sample, tokenizer, prompt_length):
    ...
    confidence_tokens = torch.tensor(
        tokenizer.encode("### Confidence", add_special_tokens=False),
